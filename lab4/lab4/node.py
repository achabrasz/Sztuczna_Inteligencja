...
class Node:
    ...
    def gini(self, left_pos, left_neg, right_pos, right_neg):
        ...
        gini = 1 - (left / (left + right)) * gini_left - (right / (left + right)) * gini_right
        return gini
    ...
    def gini_best_score(self, y, possible_splits):
        ...
        for idx in possible_splits:
            left_pos = np.sum(y[:idx+1] == 1)
            left_neg = np.sum(y[:idx+1] == 0)
            right_pos = np.sum(y[idx+1:] == 1)
            right_neg = np.sum(y[idx+1:] == 0)
            gain = self.gini(left_pos, left_neg, right_pos, right_neg)
            if gain > best_gain:
                best_gain = gain
                best_idx = idx
    ...
    def find_best_split(self, X, y, feature_subset):
        ...
        if feature_subset is not None:
            features = np.random.choice(X.shape[1], size=min(feature_subset, X.shape[1]), replace=False)
        else:
