...
def zad1_single_neuron(student_id):
    ...
    class SingleNeuron:
        ...
        def forward(self, x_data):
            ...
            out = []
            for x in x_data:
                out.append(self.f_act(np.dot(x, self.W) + self.b))
            return np.array(out)
...
def zad2_two_layer_net(student_id):
    ...
    class SimpleTwoLayerNetwork:
        ...
        def forward(self, x_data):
            layer1 = self.hidden_layer.forward(x_data)
            out = self.output_layer.forward(layer1)
            return out
...
if __name__ == '__main__':
    ...
    student_id = 193373         # Twój numer indeksu, np. 102247
